...
В проекте был выбран монолитный подход к архитектуре, так как приложение развертывается как единая программа. В дополнении к этому, такие факторы как небольшой объем проекта , запуск сервиса в докере по обращению к эндпоинтам, упаковка в докер-образ ,а так же удобство демонстрации АБ - теста позволило принять решение в пользу монолитной архитектуры без создания отдельных микросервисов.  В рамках проекта были объединены api для запроса, обучение модели и входных данных , предсказания и json-ответ для пользователя.
...
docker run -p 5000:5000 olga1789/credit-default-service:latest


RabbitMQ можно использовать для асинхронной обработки батч-заявок или очередей предсказаний.


Касательно бизес - метрик , модель должна также помогать снижать финансовые потери. Так, например, модель возвращает default_probability и на основе этой вероятности можно рассчитать ожиадаемые потери , 
Probability_of_default x Credit_limit x Loss_given_default. В результате будет рассчитан ожидаемный финансовый риск в условных денежных единицах. 
